    ...
    <footer className="relative border-t border-[var(--border-default)]">
      ...
      <div className="container-default section-padding">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          {/* Navigation */}
          <div>
            <h4 className="font-medium text-[var(--text-primary)] mb-4">Navigate</h4>
            <ul className="space-y-2">
              {footerNavItems.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-sm text-[var(--text-secondary)] hover:text-[var(--accent-oak)] transition-colors duration-300"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <div className="flex gap-4">
              {/* YouTube */}
              <a href="#" className="text-[var(--text-secondary)] hover:text-[var(--accent-oak)] transition-colors duration-300" aria-label="YouTube">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
              </a>
              {/* Instagram */}
              <a href="#" className="text-[var(--text-secondary)] hover:text-[var(--accent-oak)] transition-colors duration-300" aria-label="Instagram">
              ...
              {/* Spotify */}
              <a href="#" className="text-[var(--text-secondary)] hover:text-[var(--accent-oak)] transition-colors duration-300" aria-label="Spotify">
              ...
              {/* Apple Podcasts */}
              <a href="#" className="text-[var(--text-secondary)] hover:text-[var(--accent-oak)] transition-colors duration-300" aria-label="Apple Podcasts">
